...
class TestAddCommand:
    ...
    def test_add_json_output(self, mock_run_async, runner, mock_add_result):
        ...
        mock_run_async.return_value = {"path": "test/entry.md", "suggested_links": [], "suggested_tags": []}
        ...
        assert result.exit_code == 0
        output_data = json.loads(result.output)
        assert "path" in str(output_data)
...
class TestQuickAddCommand:
    ...
    def test_quick_add_no_content_source(self, runner):
        ...
        assert result.exit_code == 1
        assert "Provide --content, --file, or --stdin" in result.output
...
class TestPrimeCommand:
    ...
    def test_prime_basic(self, mock_mcp, runner):
        ...
        assert result.exit_code == 0
        # Check for key content from PRIME_OUTPUT
        assert "mx" in result.output.lower() or "search" in result.output.lower()

    @patch("memex.cli._detect_mcp_mode")
    def test_prime_mcp_mode(self, mock_mcp, runner):
        ...
        assert result.exit_code == 0
        # MCP mode shows minimal quick reference
        assert "Quick Reference" in result.output or "mx" in result.output

    def test_prime_force_full(self, runner):
        """Test prime with --full flag."""
        result = runner.invoke(cli, ["prime", "--full"])

        assert result.exit_code == 0
        # Full mode shows more detail
        assert "mx" in result.output.lower()

    def test_prime_force_mcp(self, runner):
        """Test prime with --mcp flag."""
        result = runner.invoke(cli, ["prime", "--mcp"])

        assert result.exit_code == 0
        # MCP mode shows minimal output
        assert "Quick Reference" in result.output or "mx" in result.output
...
class TestDetectMcpMode:
    ...
    def test_mcp_enabled_via_env_var(self, monkeypatch):
        """Test detection via MCP_SERVER_ACTIVE env var."""
        from memex.cli import _detect_mcp_mode

        monkeypatch.setenv("MCP_SERVER_ACTIVE", "1")
        assert _detect_mcp_mode() is True

    def test_mcp_not_enabled(self, monkeypatch):
...
class TestAddCommandErrorPaths:
    ...
    @patch("memex.cli.run_async")
    def test_add_category_error(self, mock_run_async, runner):
        """Test add with missing category shows error."""
        ...
        result = runner.invoke(
            cli,
            ["add", "--title", "Test", "--tags", "test", "--content", "content"],
        )
    ...
    @patch("memex.cli.run_async")
    def test_add_generic_error(self, mock_run_async, runner):
        """Test add with generic error."""
        mock_run_async.side_effect = RuntimeError("Something went wrong")
