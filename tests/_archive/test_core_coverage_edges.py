...
def test_validate_nested_path_symlink_escape(kb_root, tmp_path):
    ...
    with pytest.raises(ValueError, match="Path escapes knowledge base"):
        core.validate_nested_path("escape/file.md")
...
def test_directory_exists_and_parent_category(kb_root):
    ...
    assert core.directory_exists("development")
    assert not core.directory_exists("notes.txt")
...
async def test_add_entry_infers_category_from_tags(kb_root, index_root, monkeypatch):
    ...
    # Category inference from tags requires explicit category since no .kbcontext
...
@pytest.mark.asyncio
async def test_add_entry_with_context_default_tags(kb_root, index_root, monkeypatch):
    (kb_root / "development").mkdir()
...
async def test_hubs_and_dead_ends(kb_root, index_root, monkeypatch):
    ...
    # get_backlink_index now takes no arguments
    monkeypatch.setattr(core, "get_backlink_index", lambda: backlinks)
