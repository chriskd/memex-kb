...
class TestBuildFrontmatter:
    ...
    def test_title_with_colon(self):
        """Titles with colons are included in frontmatter."""
        metadata = EntryMetadata(
        ...
        # Title is included (simple f-string format)
    ...
    def test_tags_with_colons(self):
        """Tags containing colons are included in frontmatter."""
        metadata = EntryMetadata(
            title="Entry",
        ...
        # All tags are included (simple f-string format)
    ...
    def test_roundtrip_valid_yaml(self):
        """Frontmatter can be parsed back as valid YAML for simple content."""
        import yaml

        # Use simple content without YAML-problematic characters
        metadata = EntryMetadata(
        ...
        assert parsed["title"] == "Simple Test Entry"
...
class TestCreateNewMetadata:
    ...
    def test_minimal_metadata(self):
        ...
        # Created datetime should be close to now (UTC)
        assert isinstance(metadata.created, datetime)
        assert metadata.created.tzinfo == timezone.utc
        # Compare UTC date
        assert metadata.created.date() == datetime.now(timezone.utc).date()
...
class TestUpdateMetadataForEdit:
    ...
    def test_sets_updated_date(self, base_metadata):
        ...
        # Updated datetime should be close to now (UTC)
        assert isinstance(updated.updated, datetime)
        assert updated.updated.tzinfo == timezone.utc
        assert updated.updated.date() == datetime.now(timezone.utc).date()
