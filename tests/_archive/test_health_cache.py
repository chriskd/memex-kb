...
class TestHealthCacheIntegration:
    ...
    @pytest.mark.asyncio
    async def test_health_skips_parse_errors(self, tmp_path, monkeypatch):
        """Entries with parse errors are skipped in health check."""
        from memex import core
        ...
        # Should still count valid entries (broken ones are skipped)
...
class TestParseErrorsFunction:
    ...
    @pytest.mark.asyncio
    async def test_health_reports_broken_links_with_typos(
        self, tmp_path, monkeypatch
    ):
        """Broken links are reported even when similar valid targets exist."""
        from memex import core
        ...
        assert broken["source"] == "source.md"
...
"""
    ...
    @pytest.mark.asyncio
    async def test_health_counts_entries_with_and_without_descriptions(
        self, tmp_path, monkeypatch
    ):
        """Health check counts all entries regardless of description."""
        from memex import core
    ...
    @pytest.mark.asyncio
    async def test_health_with_multiple_entries(self, tmp_path, monkeypatch):
        """Health check processes multiple entries correctly."""
        from memex import core
