...
class TestAddTool:
    ...
    @pytest.mark.asyncio
    async def test_add_returns_suggested_links(self, kb_root, index_root):
        """Happy path: Adding entry returns suggested links."""
        # Create related entry
        _create_entry(
            kb_root / "development" / "existing.md",
            "Related Python Guide",
            ["python", "guide"],
    ...
    @pytest.mark.asyncio
    async def test_add_returns_suggested_tags(self, kb_root, index_root):
        """Happy path: Adding entry returns suggested tags."""
        result = await _call_tool(
            server.add_tool,
            title="Test Entry",
            content="Content",
